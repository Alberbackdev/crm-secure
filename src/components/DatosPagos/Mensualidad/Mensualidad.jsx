...
function Mensualidad({ selectMeses }) {
  ...
  const handleChange = (event, month) => {
    ...
    const isMonthInTheArray =
      name === "mes" && data.find((mes) => mes?.mes === value);

    if (isMonthInTheArray) {
      // Condicion para comprobar si seleccionan un mes dos veces, en caso de que si, removerlo.
    ...
    if (name === "mes") {
      // Condicion para agregar un mes
      return setData([...data, { mes: value, value: "0.00" }]);
    }
    // Agregar monto a un mes.
    const putMontoAMes = data.map((mes) =>
      mes.mes === month ? { ...mes, value: value } : mes
    );
  ...
  useEffect(() => {
    ...
    for (const elemento of monthsList) {
      // data = {mes: 'Enero', value: '200'}
      const comprobarMesPagado = data.find(
        (month) => month.mes === elemento.nombreMes
      );

      elemento.nombreMes === comprobarMesPagado?.mes
        ? dataTransformadaAGuardar.push({
            nombreMes: comprobarMesPagado.mes,
            monto: comprobarMesPagado.value,
          })
        ...
        {monthsList.map((month, i) => (
          <div className={style.cardMensualidad} key={i}>
            ...
            <input
              ...
              onChange={(event) => handleChange(event, month.nombreMes)}
              disabled={
                !data.some((iteration) => iteration?.mes === month.nombreMes) ||
                (month.monto && valuesPago.updatingPagoData)
              }
            />
          </div>
        ))}
